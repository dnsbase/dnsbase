...
type SGet a = RSE SGetError SGetEnv SGetState a

runSGet :: SGet a -> SGetEnv -> SGetState -> Either SGetError (a, SGetState)
runSGet = runRSE

evalSGet :: SGet a -> SGetEnv -> SGetState -> Either SGetError a
evalSGet = evalRSE

failSGet :: SGetError -> SGet a
failSGet = throwRSE
...
getIPv4Net n | n >= 0 && n <= 4 =
    getNByteString n >>= \ (BS fp _) -> pure $! unsafePerformFPIO fp \ptr -> do
        allocaBytesAligned 4 4 $ \buf -> do
            ...
            pure $ toIPv4w w
getIPv4Net _ = failSGet "invalid IPv4 net length"
...
getIPv6Net _ = failSGet "invalid IPv6 net length"
...
getVarWidthSequence getOne = fitSGet <$> id <*> go
  where
    go n | n > 0 = do
      ...
      (x : ) <$> go (n - used)
    go 0 = pure []
    go _ = failSGet "Last sequence element read past buffer limit"
