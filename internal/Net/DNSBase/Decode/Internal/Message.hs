...
getMessage dm om = do
    ...
    if | hasAnyFlags TCflag $ p_dnsMsgFl phd
       ...
       | otherwise -> do
            if | q:_ <- queries -> () <$ setLastOwner (dnsQuName q)
               | otherwise      -> pure ()
            answers <- getRRs dm Nothing anCount
            authrrs <- getRRs dm Nothing nsCount
            addnrrs <- getRRs dm (Just om) arCount
            case partition isOpt addnrrs of
                ...
                _ -> failSGet "Multiple or bad additional section OPT records"
  where
    isOpt :: RR -> Bool
...
getQueries n = replicateM n getQuery
  where
    ...
    getQuery = Question <$> getDomain <*> getType <*> getClass
      where
