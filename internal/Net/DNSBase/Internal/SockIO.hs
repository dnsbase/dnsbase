...
import Net.DNSBase.Decode.Internal.State (decodeAtWith)
...
--
receiveUDP :: RDataMap -> OptionMap -> Word16 -> Socket -> DNSIO DNSMessage
receiveUDP dm om maxudp sock = do
    bs <- withExceptT wrapError $ recv' sock bufsiz
    Elapsed (Seconds now) <- lift timeCurrent
    case decodeAtWith now True (getMessage dm om) bs of
        Left e    -> throwE $ DecodeError e
...
--
receiveTCP :: RDataMap -> OptionMap -> Socket -> DNSIO DNSMessage
receiveTCP dm om sock = do
...
recvDNS sock len = withExceptT (NetworkError . NetworkFailure) $ recv1
  where
