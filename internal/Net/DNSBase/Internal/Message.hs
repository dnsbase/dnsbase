...
data DNSMessage = DNSMessage
    ...
    , dnsMsgQu :: [Question]  -- ^ The question name, type, class
    , dnsMsgAn :: [RR]      -- ^ Answers
...
data Question = Question {
    dnsQuName  :: Domain  -- ^ The requested domain name
  , dnsQuType  :: RRTYPE  -- ^ The query type
  , dnsQuClass :: RRCLASS -- ^ The query class
  } deriving (Eq, Show)
...
putQuestion Question{..} = do
    putDomain dnsQuName
    put32 $ fromIntegral @Word16 (coerce dnsQuType) `unsafeShiftL` 16 .|.
            fromIntegral @Word16 (coerce dnsQuClass)
