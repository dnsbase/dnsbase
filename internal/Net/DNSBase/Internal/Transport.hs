...
lookupRawCtl rlv qctls dom qclass qtype
  | isIllegalQT qtype = throwE $ UserError $ InvalidQueryType qtype
  | otherwise = case genPairs rlv of
      (ai, gen) :| [] -> resolveOne ai gen retry tmout q ctls conf
      ngs             -> resolveSeq ngs retry tmout q ctls conf
  where
    conf           = resolvconf $ resolvseed rlv
    tmout          = rcTimeout conf
    retry          = rcRetries conf
    ctls           = qctls <> rcQryCtls conf
    q              = Question dom qtype qclass
...
resolveSeq :: NonEmpty (AddrInfo, IO QueryID) -> UdpLookup
resolveSeq ngs retry tmout q qctls conf = loop ngs
  where
    loop ((ai, gen) :| []) = resolveOne ai gen retry tmout q qctls conf
    loop ((ai, gen) :| (ag' : agt)) =
        resolveOne ai gen retry tmout q qctls conf
            `catchE` const (loop (ag' :| agt))

-- UDP attempts must use the same ID and accept delayed answers
-- but we use a fresh ID for each TCP lookup.
--
resolveOne :: AddrInfo -> IO QueryID -> UdpLookup
resolveOne ai gen retry tmout q qctls conf = do
    ident <- lift gen
    udpLookup ai ident retry tmout q qctls conf

----------------------------------------------------------------

ioErrorToDNSError :: AddrInfo -> String -> DNSError -> DNSIO DNSMessage
ioErrorToDNSError ai protoName = \case
    NetworkError (NetworkFailure err) ->
      let loc  = protoName ++ "@" ++ show (addrAddress ai)
...
--
udpLookup :: AddrInfo -> QueryID -> UdpLookup
udpLookup ai ident retry tmout q qctls conf =
    case encodeQuestion ident qctls q of
      Left err -> throwE err
      Right qry -> do
        flip catchE (ioErrorToDNSError ai "udp") $
            bracket' do udpOpen ai
                     do close
  ...
  where
      ...
      | otherwise       = do
          ...
          case mres of
              ...
              Just res -> do
                      ...
                      if | tc -> tcpLookup ai ident tmout q qctls conf
    ...
    getAns ctls sock = do
        resp <- receiveUDP dm om maxsz sock
        if checkResp q ident resp
        then return resp
        else getAns ctls sock
      where
        dm = rcDataMap conf
        om = rcOptnMap conf
...
tcpLookup :: AddrInfo -> QueryID -> TcpLookup
tcpLookup ai ident tmout q qctls conf =
    flip catchE (ioErrorToDNSError ai "tcp") $ do
        res <- bracket' do tcpOpen addr
                        do close
                        do perform qctls
        let rc = dnsMsgRC res
            eh = dnsMsgEx res
        ...
        case encodeQuestionLP ident ctls q of
          ...
          Right qry -> do
              mres <- timeout' tmout $ do
                  ...
                  sendTCP sock qry
                  receiveTCP dm om sock
              case mres of
                  Nothing  -> throwE $ NetworkError TimeoutExpired
                  Just res -> maybe (pure res) throwE (checkRespM q ident res)
