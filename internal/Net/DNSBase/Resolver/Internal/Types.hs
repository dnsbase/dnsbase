module Net.DNSBase.Resolver.Internal.Types
     ...
     -- * Resolver configuration
       ResolvSeed(..)
     , Resolver(..)
     -- ** Resolver configuration structure components
     , Remote(..)
     , ConfigSrc(..)
     , ResolverConf(..)
...
-- | ADT representing either a host name or an IP address
data Remote =
     Remote { remoteNS :: String
            , remotePt :: Maybe PortNumber
            }

-- | Resolver configuration file, or list of addresses/hostnames
data ConfigSrc = SourceFile FilePath
               | HostList (NonEmpty Remote)
...
data ResolverConf = ResolverConf
    ...
    , rcSource  :: ConfigSrc     -- ^ Resolver configuration file or list of remote sources
    , rcQryCtls :: QueryControls -- ^ Query and Codec Controls
    , rcDataMap :: RDataMap      -- ^ Known RData codecs
    , rcOptnMap :: OptionMap     -- ^ Known EDNS Option codecs
    }


--- BEGIN copy from dns library
----------------------------------------------------------------
