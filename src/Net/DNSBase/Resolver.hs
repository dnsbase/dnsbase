...
pattern ResolverConfSource :: ConfigSrc -> ResolverConf -> ResolverConf
...
-- Get or Set Resolver Rdata map (built-in defintions always present)
{-# COMPLETE ResolverConfRDataMap #-}
pattern ResolverConfRDataMap :: RDataMap -> ResolverConf -> ResolverConf
pattern ResolverConfRDataMap d rc <- rc@ResolverConf {rcDataMap = d} where
    ResolverConfRDataMap d rc = rc {rcDataMap = baseCodecs <> d}

-- Get or Set Resolver EDNS option map (built-in definitions always present)
{-# COMPLETE ResolverConfOptionMap #-}
pattern ResolverConfOptionMap :: OptionMap -> ResolverConf -> ResolverConf
pattern ResolverConfOptionMap d rc <- rc@ResolverConf {rcOptnMap = d} where
    ResolverConfOptionMap d rc = rc {rcOptnMap = baseOptions <> d}
...
resolverCodecParamUpdate _ (more :: CodecOpts a) rc
    ...
    , kvm' <- IM.insert key entry kvm = Just $! rc { rcDataMap = kvm' }
    | otherwise                       = Nothing
  where
...
makeResolvSeed conf = ResolvSeed conf <$> findAddresses
  where
    findAddresses :: DNSIO (NonEmpty AddrInfo)
    findAddresses = case rcSource conf of
        HostList rs     -> mkRemoteAddrs rs
        SourceFile file -> getDefaultDnsServers file >>= mkAddrs

    mkRemoteAddrs = fmap join . mapM (\(Remote h mp) -> makeAddrInfo (Just h) mp)

    mkAddrs (l:ls) = fmap join $ forM (l :| ls) (flip makeAddrInfo Nothing . Just)
...
makeAddrInfo :: Maybe HostName -> Maybe PortNumber -> DNSIO (NonEmpty AddrInfo)
makeAddrInfo maddr mport = do
    let flgs = (if maybe True isAddr maddr
                  then (AI_NUMERICHOST :)
                  else id)
    ...
    withExceptT BadNameserver (getAddrInfo' hints maddr serv) >>= \ case
        x : xs -> return $ x :| xs
        _      -> let host = fromMaybe "<unspecified>" maddr
...
makeResolver resolvseed = do
    genPairs <- forM (nameservers resolvseed) \ ai -> do
        gen <- fmap getRandom $ C.drgNew >>= I.newIORef
        pure (ai, gen)
    pure Resolver{..}
  where
...
confTypeNames cnf =
  ...
  where
    cnfMap (ResolverConf{..}) =
        M.fromList $ map (uncurry mkPair) $ IM.toList rcDataMap
      where
