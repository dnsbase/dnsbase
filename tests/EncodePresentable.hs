...
nsec3owner :: Domain
nsec3owner = $$(dnLit8 $ BS.unpack $ encodeBase32Unpadded' (B.pack [0..19]) <> ".example.com")
...
soa = mkRD $ T_SOA { soaMname = $$(dnLit8 "nsa.example.com")
                   , soaRname = $$(mbLit8 "postmaster@example.com")
                   , soaSerial = 1
                   , soaRefresh = 3600      -- 1H
                   , soaRetry   = 300       -- 5m
...
afs = mkRD $ T_AFSDB 1 $$(dnLit8 "afsdb.example.org")
hi = mkRD $ T_HINFO "alpha" "NetBSD 8"
txt = mkRD $ T_TXT $ "v=spf1" :| [" include:_spf.example.com", " ~all"]
ptr = mkRD $ T_PTR $$(dnLit8 "www.example.org")
nsec = mkRD $ T_NSEC $$(dnLit8 "www.example.org")
  ...
  where
    loop n
        | n > 0 =
          ...
          . presentLn soa
          . presentLn rrsig
          . presentLn (mkRD $ T_NS $$(dnLit8 "nsa.example.org."))
          . presentLn (mkRD $ T_CNAME $$(dnLit8 "www.example.org."))
          . presentLn (mkRD $ T_DNAME $$(dnLit8 "example.net."))
          . presentLn (mkRD $ T_MX 10 $$(dnLit8 "smtp.example.org."))
          . presentLn (mkRD $ T_SRV 100 0 389 $$(dnLit8 "ldap.example.org."))
          . presentLn afs
          . presentLn hi
          . presentLn txt
          ...
          . presentLn (RR $$(dnLit8 "_25._tcp.mx1.example.com")
